...
把一個 change 的**全部**（凍結的規格、WBS 那一項、DECISIONS 裡提到它的整節、
每個 slice PR 的完整 diff 與說明）打成一包，平行送兩個不是寫它的模型，各自獨立審一次。
