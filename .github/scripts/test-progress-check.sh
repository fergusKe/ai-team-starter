...
| 里程碑 | 目標週 | 驗收結果 | 靠哪些 |
|---|---|---|---|
| M1 一個人跑完主流程 | W2 | 進來、看到、做完一件事 | APP-C01、APP-P03 |
...
run_all_has "表格列前面有空白：那一列還看得見" "APP-C01"
...
# 現在有兩層：`Pd` 類折成 `-`（於是 `APP﹣Q99` 直接被當成 `APP-Q99` 驗），
...
baseline
mkchange app-c01-ui
mkarchived app-c01-api
run_field_has "change 清單依 ID 排序（不是字典插入順序）" "APP-C01" "changes" "'app-c01-api', 'app-c01-ui'"

# **看得見不等於算得對。** 上面兩條都綠的時候，狀態仍然是錯的：
#
#   APP-C01  兩個 change 都已封存   有分支   app-c01-api +1
#
# 根因是 `change_for()` 回傳 `"app-c01-api +1"` 這個**顯示字串**，
...
baseline
mkarchived app-c01-api app-c01-ui
run_field_has "兩個 change 都封存了就是已封存（不是有分支）" "APP-C01" "state" "已封存"
...
mkarchived app-c01-api
mkchange app-c01-ui
run_field_has "一個封存、一個還在做：不算已封存" "APP-C01" "state" "規格已合併"
...
baseline
mkchange app-c01-api app-c01-ui
mkremote feat/app-c01-ui--slice
run_field_has "第二個 change 有實作分支：項目就是實作中" "APP-C01" "state" "實作中"
...
render
run 0 "產生之後 --check 綠" ""
run_block have "區塊列出有進度的項目" "| APP-C01 | 規格已合併 |"
...
mkmarkers
render
run_block absent "混合狀態不准在區塊裡變成已封存" "| APP-C01 | 已封存 |"
run_block have "混合狀態在區塊裡是規格已合併" "| APP-C01 | 規格已合併 |"
...
mkmarkers
render
run_block have "兩個都封存時區塊裡是已封存" "| APP-C01 | 已封存 |"

# **依狀態排序，不依 ID。** 讀的人問的是「哪些做完了」——
# 按 ID 排的話那幾項會被外部缺口埋在中間。
#
# fixture 要能分辨兩種排法：`DEP-G01` 的 ID 排在最後、狀態（已封存）排在最前，
# `APP-O10`（常態）反過來。按 ID 排 → APP-O10 在前；按狀態排 → DEP-G01 在前。
baseline
mkarchived dep-g01-x
mkmarkers
render
...
sys.exit(0 if rows[:2] == ["DEP-G01", "APP-O10"] else 1)
ORDER
then echo "✓ 區塊依狀態排序，不依 ID"; PASS=$((PASS + 1))
else echo "✗ 區塊沒有依狀態排序（已封存的 DEP-G01 要排在常態的 APP-O10 前面）"; bump_fail; fi
...
edit "APP-O10 | 文件維護" "APP-O10 | 文件保養"
render
...
baseline
mkchange app-c01-x
mkremote feat/app-c01-x--slice
mkmarkers
render
run_block absent "區塊裡不准有「實作中」" "| APP-C01 | 實作中 |"
run_block have "同一項在區塊裡是耐久狀態「規格已合併」" "| APP-C01 | 規格已合併 |"
...
mkmarkers
render
run_block absent "區塊不複製項目名稱" "應用骨架"

# **沒有進度的項目不列出來** —— 161 項全列進去，區塊會比表本身還長，
# 而且「未開始」是預設值，列出來不帶任何資訊。
run_block absent "未開始的項目不列出來" "| APP-P03 |"
...
edit "APP-C01 | 應用骨架" "APP-C01 | 應用骨架改名"
render
...
baseline
edit "| ID | 項目 |" "| **ID** | 項目 |"
run_all_has "表頭的 ID 被加粗：照樣認得出來" "APP-C01"
...
run_expr "正本四欄真的進了 json（含 covers 與目標週）" \
  "meta['schema']=='canonical' and meta['coverage_status']=='available' \
   and len(m)==1 and m[0]['id']=='M1' and m[0]['covers']==['APP-C01','APP-P03'] \
...
edit "| 里程碑 | 目標週 | 驗收結果 | 靠哪些 |
|---|---|---|---|
| M1 一個人跑完主流程 | W2 | 進來、看到、做完一件事 | APP-C01、APP-P03 |" "還沒想好，先留個位置。"
edit "| 這一項 | 依賴 |
|---|---|
| \`APP-P03\` 清單 | \`APP-C01\` 骨架 |" "（跨項依賴表也先拿掉，讓這一節只剩標題與一句話。）"
...
edit "| 里程碑 | 目標週 | 驗收結果 | 靠哪些 |
|---|---|---|---|
| M1 一個人跑完主流程 | W2 | 進來、看到、做完一件事 | APP-C01、APP-P03 |" "（正本那張表先拿掉，只留下面那張兩欄的跨項依賴表。）"
...
baseline
edit "| M1 一個人跑完主流程 | W2 | 進來、看到、做完一件事 | APP-C01、APP-P03 |" \
     "| M1 一個人跑完主流程 | W2 | 進來、看到、做完一件事 |"
run 1 "里程碑表裡欄數對不上要紅" "表頭是 4 欄"

# 「靠哪些」指向不存在的項目 —— 重排群組之後最容易斷的就是這種。
baseline
edit "APP-C01、APP-P03 |" "APP-C01、APP-Z11 |"
run 1 "里程碑的「靠哪些」指向不存在的 ID 要紅" "APP-Z11"

baseline
edit "APP-C01、APP-P03 |" "APP-C01、APP-C01 |"
run 1 "「靠哪些」裡重複引用同一個 ID 要紅" "重複的 ID"

baseline
edit "| M1 一個人跑完主流程 | W2 | 進來、看到、做完一件事 | APP-C01、APP-P03 |" \
     "| M1 一個人跑完主流程 | W2 | 進來、看到、做完一件事 | APP-C01 |
| M1 又一個 M1 | W3 | 不該過 | APP-P03 |"
run 1 "里程碑 ID 重複要紅" "重複的里程碑 ID"

# 後面章節的 `| **W9** | 41 |`（例如一張每週負荷參考表）不准被吃成里程碑。
# 舊的解析器掃到檔尾，所以它會變成一個叫「41」的里程碑。
baseline
edit "| \`APP-P03\` 清單 | \`APP-C01\` 骨架 |" "| \`APP-P03\` 清單 | \`APP-C01\` 骨架 |
...
# same_week：把 APP-C01 兩列都挪到 W2，跟 APP-P03 同一週。
same_week() {
...
baseline
edit "| 決策≤W1 | — | \`外部-缺\` | Alarm" "| 決策≤W1 | — | DEP-G01 \`外部-缺\` | Alarm"
run 1 "阻塞欄指向自己要紅" "阻塞欄指向自己"

# 缺口擋缺口繞成一圈：沒有規則禁止缺口擋缺口，但繞成一圈就誰都等不到。
baseline
edit "| 決策≤W1 | — | \`外部-缺\` | Alarm" "| 決策≤W1 | — | DEP-G02 \`外部-缺\` | Alarm"
edit "| APP-C01 | 應用骨架 |" "| DEP-G02 | 另一個缺口 | 去問。**【沒答案就】**先不做 | 決策≤W1 | — | DEP-G01 \`外部-缺\` | Alarm｜測試用 |
| APP-C01 | 應用骨架 |"
run 1 "阻塞欄的缺口互相擋（環）要紅" "阻塞欄繞成一圈"
run_expr "環與缺口→缺口候選邊真的進了 json" \
  "g['blockers']['cycles']==[['DEP-G01','DEP-G02']] and g['blockers']['gap_to_gap_candidates']==2"
...
mkarchived app-p03-list
run 1 "已封存的項目還掛著「待裁決」缺口要紅" "阻塞欄卻還指向「待裁決」的 DEP-G01"
...
baseline
xdep "| APP-P03 | APP-C01 | 全部 | 清單要先有骨架 |"
...
baseline
xdep "| APP-C01 | APP-P03 | 全部 | 骨架反過來依賴清單 |"
run 1 "〈跨項依賴〉逆序要紅（W1 依賴 W2）" "被依賴的要在同一週或更早"
run_expr "逆序真的進了 json" \
  "g['cross_deps']['reversed'][0]['src']=='APP-C01' and g['cross_deps']['reversed'][0]['dep']=='APP-P03'"

# 同週合法：一週是排程桶，不是順序。
baseline
same_week
xdep "| APP-C01 | APP-P03 | 全部 | 同一週 |"
run 0 "〈跨項依賴〉同週合法" ""
run_expr "同週那條真的被評估了（不是被跳過才綠）" \
  "g['cross_deps']['evaluated']==1 and g['cross_deps']['reversed']==[]"

baseline
xdep "| APP-C01 | APP-P03、APP-O10 | 任一 | 兩個都來不及 |"
...
mkarchived app-p03-list
xdep "| APP-P03 | APP-C01 | 全部 | 清單要先有骨架 |"
run 1 "〈跨項依賴〉：已封存而前置還沒完成要紅" "但它依賴的 APP-C01 還沒完成"

baseline
mkarchived app-p03-list app-c01-shell
xdep "| APP-P03 | APP-C01 | 全部 | 清單要先有骨架 |"
run 0 "對照：前置也封存了就是綠的" ""
...
baseline
xdep "| APP-P03 | APP-C01 → APP-O10 | 全部 | 鏈 |"
run 1 "依賴欄不收 →（A → B 是兩件事，拆成兩列）" "不收 \`→\` 與 \`或\`"

baseline
xdep "| APP-P03 | APP-C01 或 APP-O10 | 全部 | 或 |"
run 1 "依賴欄不收「或」（寫在關係欄的「任一」）" "不收 \`→\` 與 \`或\`"

baseline
xdep "| APP-P03 的「搜尋與篩選」 | APP-C01 | 全部 | 只指其中一條驗收 |"
run 1 "只指某一條驗收的依賴不准寫在項目層級" "只指某一條驗收"

baseline
xdep "| APP-P03 | APP-C01 | 都要 | 關係寫錯 |"
run 1 "關係只能是「全部」或「任一」" "只能是 \`全部\` 或 \`任一\`"

baseline
xdep "| APP-P03 | APP-C01 骨架 | 全部 | 依賴欄夾了說明 |"
run 1 "依賴欄只能是 ID（說明寫在說明欄）" "只能是工作項目 ID"

baseline
xdep "| APP-P03 | APP-C01 | 全部 |"
...
| 這一項 | 依賴 |"
edit "| \`APP-P03\` 清單 | \`APP-C01\` 骨架 |" "| \`APP-C01\` 骨架 | \`APP-P03\` 清單（反過來，逆序） |"
...
baseline
edit "| APP-C01 | 應用骨架 | 專案骨架 | W1 | 3 | | |" "| APP-C01 | 應用骨架 | 專案骨架 | W1 | 3 | | Done｜PR #1 |"
edit "| APP-P03 | 清單元件 | 列表與翻頁 | W2 | 5 | | |" "| APP-P03 | 清單元件 | 列表與翻頁 | W2 | 5 | | Done｜PR #2 |"
xdep "| APP-P03 | APP-C01 | 全部 | 清單要先有骨架 |"
run 0 "兩項都完成：綠" ""
...
baseline
edit "| APP-C01 | 應用骨架 | 專案骨架 | W1 | 3 | | |" "| APP-C01 | 應用骨架 | 專案骨架 | W1 | 3 | | Cancelled｜改用別的做法 |"
edit "| APP-P03 | 清單元件 | 列表與翻頁 | W2 | 5 | | |" "| APP-P03 | 清單元件 | 列表與翻頁 | W2 | 5 | | Done｜PR #2 |"
xdep "| APP-P03 | APP-C01 | 全部 | 清單要先有骨架 |"
run 1 "已完成而前置已取消：講清楚是「已取消、改寫這一列」" "它依賴的 APP-C01 已取消（改用別的做法完成的話"
run_absent 1 "…而且不說它「還沒完成」" "APP-C01 還沒完成"


# 「靠哪些」寫群組：群組不是涵蓋，原本被靜靜吞掉、可追溯性報它的成員未涵蓋。
baseline
edit "| APP-C01、APP-P03 |" "| APP-C、APP-P03 |"
run 1 "「靠哪些」寫群組要紅（逐項列出）" "寫了群組 APP-C"
...
baseline
xdep "| APP-C01 | DEP-G01 | 全部 | 應該寫在阻塞欄 |"
run 1 "〈跨項依賴〉的依賴是缺口要紅" "DEP-G01 是缺口"

# 用 change 做掉的缺口（全部封存）也是已裁決 —— 有 change 的項目不准標 Done。
baseline
anchor
mkarchived dep-g01-search
...
baseline
run 0 "對照：不加 --week 有「哪些項目依賴外部」（DEP-G01 擋著 W2 的 APP-P03）" "哪些項目依賴外部"
...
baseline
edit "| APP-O10 | 文件維護 |" "| APP-O11 | 佈署 | 等平台 | — | — | | Pending｜平台還沒開 |
| APP-O10 | 文件維護 |"
xdep "| APP-P03 | APP-O11、APP-C01 | 全部 | 要先有平台 |"
run 1 "依賴一個沒排週次的項目要紅（先排週次）" "APP-O11 沒有工作週次，排不出先後"

baseline
xdep "| APP-P03 | APP-O10 | 全部 | 依賴常態維運 |"
run 1 "依賴一個常態（Regular）項目要紅（拆出有週次的一次性項目再依賴）" "APP-O10 沒有工作週次，排不出先後"
...
baseline
xdep "| DEP-G01 | APP-C01 | 全部 | 裁決要等骨架 |"
run 1 "這一項是缺口要紅（決策期限不能當起始週）" "這一項 DEP-G01 是缺口"
...
# S2：這一項是常態：永遠沒有起點。
baseline
xdep "| APP-O10 | APP-C01 | 全部 | 維運要先有骨架 |"
run 1 "這一項是常態要紅（拆出有週次的一次性項目）" "這一項 APP-O10 是常態"

# S3：這一項還沒排週次：不紅，但 --check 列出來——就算同一張表別的邊有被評估。
baseline
edit "| APP-O10 | 文件維護 |" "| APP-O11 | 佈署 | 等平台 | — | — | | Pending｜平台還沒開 |
| APP-O10 | 文件維護 |"
xdep "| APP-P03 | APP-C01 | 全部 | 正常的一條 |
| APP-O11 | APP-P03 | 全部 | 佈署要等清單 |"
run 0 "這一項還沒排週次：不紅，但 --check 點名（別的邊有評估也要講）" "APP-O11（第 [0-9]* 行）還沒排週次"
...
edit "| APP-O11 | 佈署 | 等平台 | — |" "| APP-O11 | 佈署 | 等平台 | W1 |"
run 1 "…排上週次之後同一列自動被驗（W1 依賴 W2 要紅）" "APP-O11（最早 W1）依賴 APP-P03（最晚 W2）"

# S5：已完成卻沒有工作週次：堵住「沒排週次就直接做完」這條沒被驗過的綠燈路徑。
# 依賴那一邊也做完了，D7 不會叫 —— 紅的只能是這一條。
baseline
mkarchived app-c01-skeleton
edit "| APP-O10 | 文件維護 |" "| APP-O12 | 治理文件 | 寫好了 | — | — | | Done｜PR #1 |
| APP-O10 | 文件維護 |"
xdep "| APP-O12 | APP-C01 | 全部 | 文件要等骨架 |"
run 1 "這一項已完成卻沒有週次要紅（補實際週次，或刪掉這一列）" "APP-O12 已完成，卻沒有工作週次"
...
edit "| APP-O10 | 文件維護 |" "| APP-C03 | 舊做法 | 改用別的了 | W1 | 1 | | Cancelled｜改用 APP-C01 的做法 |
| APP-O10 | 文件維護 |"
xdep "| APP-C03 | APP-P03 | 全部 | 原本 W1 等 W2，逆序 |"
run 0 "這一項已取消：逆序也不紅（不受排程約束），但 --check 點名" "APP-C03（第 [0-9]* 行）已取消"
...
baseline
edit "| APP-O10 | 文件維護 |" "| APP-C03 | 舊做法 | 改用別的了 | W1 | 1 | | Cancelled｜改用 APP-C01 的做法 |
| APP-O11 | 佈署 | 等平台 | — | — | | Pending｜平台還沒開 |
| APP-O10 | 文件維護 |"
xdep "| APP-O11 | APP-C03 | 全部 | 依賴的已取消 |"
...
| APP-O11 | 佈署 | 等平台 | — | — | | Pending｜平台還沒開 |
| APP-O10 | 文件維護 |"
xdep "| APP-P03 | APP-C01 | 全部 | 評估 |
| APP-O11 | APP-P03 | 全部 | 待排 |
| APP-C03 | APP-C01 | 全部 | 這一項已取消 |"
...
edit "| 這一項 | 依賴 |
|---|---|
| \`APP-P03\` 清單 | \`APP-C01\` 骨架 |" "已知的跨項依賴（改動時要一起看）：
