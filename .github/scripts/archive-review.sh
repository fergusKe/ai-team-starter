...
MIN_VERIFIED=2      # 樣本內，經人工判定「已驗證」且該 change 有回審紀錄（修了才會有第二輪）的需修正 ≥ 這個數
...
r1ok = {}                                             # id → {model: row}，只收回答了的第一輪
for r in rev:
    if r["round"] == 1 and r.get("ok", True): r1ok.setdefault(r["id"], {})[r["model"]] = r   # 舊 row 沒有 ok 欄：當時只有回答了才會寫 row
first = {}                                            # id → 第一次第一輪的時間（決定樣本順序）
for r in rev:
    if r["round"] == 1: first.setdefault(r["id"], r["ts"])
both = [i for i in sorted(first, key=first.get) if {"codex", "gemini"} <= set(r1ok.get(i, {}))]
half = [i for i in first if i not in both]
cohort = both[:N]
print(f"條件：最早 {N} 個雙模型樣本內 已驗證且回審過 ≥{MINV}、誤報率 ≤{MAXFP}%、等待 P90 ≤{MAXP90} 秒")
...
rere = {r["id"] for r in rev if r["round"] == 2 and r.get("ok", True)}
ok = sum(1 for j in judg if j["verdict"] == "已驗證" and j["id"] in rere)
ok_norere = sum(1 for j in judg if j["verdict"] == "已驗證" and j["id"] not in rere)
...
print(f"需修正共 {total_nf} 條：已驗證且回審過 {ok}、已驗證但沒回審 {ok_norere}、誤報 {fp}、**還沒判定 {pending}**"
...
EXCL=(-- . ':(exclude)openspec/changes/archive')
for f in package-lock.json yarn.lock pnpm-lock.yaml bun.lockb Cargo.lock poetry.lock uv.lock Pipfile.lock go.sum Gemfile.lock composer.lock; do EXCL+=(":(exclude)$f"); done
...
row() { # row <model> <t0> <rc> [session]
  ledger "{\"kind\":\"review\",\"id\":\"$ID\",\"round\":$ROUND,\"model\":\"$1\",\"seconds\":$(( $(date +%s) - $2 )),\"need_fix\":$(count "$OUT/$1.md" 需修正),\"risk\":$(count "$OUT/$1.md" 可接受風險),\"fp\":$(count "$OUT/$1.md" 誤報候選),\"ok\":$(answered "$3" "$OUT/$1.md"),\"session\":\"${4:-}\"}"
  [ "$(answered "$3" "$OUT/$1.md")" = true ] || echo "✗ $1 沒有回答（rc=${3}；看 $OUT/$1.err）—— 這次不算數" >&2
}
run_codex() {
  local t0; t0=$(date +%s)
